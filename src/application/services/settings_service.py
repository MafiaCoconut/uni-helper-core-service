...
class SettingsService:
    ...
    async def menu_settings(self, callback, user_id: int, locale: str):
        await self.settings_use_case.menu_settings(callback, user_id=user_id, locale=locale)

    async def change_locale(self, callback, user_id: int, new_locale: str):
        await self.settings_use_case.change_locale(callback, user_id=user_id, new_locale=new_locale)

    async def menu_change_mailing_time(self, callback, locale: str):
        await self.settings_use_case.menu_change_mailing_time(callback, locale=locale)

    async def change_mailing_time(self, callback, user_id: int, locale: str, new_mailing_time: str):
        await self.settings_use_case.change_mailing_time(callback, user_id=user_id,
                                                         new_mailing_time=new_mailing_time, locale=locale)

    async def change_mailing_status(self, callback, user_id: int, locale: str):
        await self.settings_use_case.change_mailing_status(callback, user_id=user_id, locale=locale)

    async def menu_change_canteen(self, callback, user_id: int, locale: str):
        await self.settings_use_case.menu_change_canteen(callback, user_id=user_id, locale=locale)

    async def change_canteen(self, callback, user_id: int, locale: str, new_canteen_id: int):
        await self.settings_use_case.change_canteen(callback, user_id=user_id,
